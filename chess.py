...
import time
from Contract_Deployment import store_result,get_game_status
...
class Chess(object):
    ...
    def capture_piece(self, turn, chess_board_coord, piece_coord):
        ...
        if p[0] == "white_king":
            self.winner = "Black"
            # Changes made here by me:
            print("Black wins")
            # Adding the function to add the status to the Blockchain
            store_result(f'{self.winner} is the current winner!')
            print("The Black winning status has been addeed to the blockchain!")
        elif p[0] == "black_king":
            self.winner = "White"
            print("White wins")
            # Adding the function to add the status to the Blockchain
            store_result(f'{self.winner} is the current winner!')
            print("The White winning status has been addeed to the blockchain!")
